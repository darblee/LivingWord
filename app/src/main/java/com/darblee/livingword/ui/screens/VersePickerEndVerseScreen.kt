    ...
    Scaffold { paddingValues ->
        Column(
            ...
            Column(
                ...
                val verses: List<Int> = remember(book, chapter, startVerse) {
                ...
                if (verses.isNotEmpty()) {
                    LazyVerticalGrid(
                        ...
                        val onVerseSelected: (Int) -> Unit = { selectedVerse ->
                            // Return BibleVerseRef with range
                            ...
                            val resultJson = kotlinx.serialization.json.Json.encodeToString(BibleVerseRef.serializer(), result)
                            ...
                            targetBackStackEntry?.savedStateHandle?.set(com.darblee.livingword.Global.VERSE_RESULT_KEY, resultJson)
                        ...
                        items(verses) { verse ->
                            EndVerseButton(verse = verse, onClick = onVerseSelected)
                        }
                    }
                } else {
                    Text(
...
/**
 * Composable function that displays a button for selecting an end verse.
 *
 * This button, when clicked or double-clicked, triggers the respective lambda function
 * with the verse number as a parameter.
 *
 * @param verse The verse number to be displayed on the button.
 * @param onClick A lambda function to be executed when the button is clicked.
 *                It takes the verse number (Int) as a parameter.
 */
@Composable
private fun EndVerseButton(verse: Int, onClick: (Int) -> Unit) {
    Button(
